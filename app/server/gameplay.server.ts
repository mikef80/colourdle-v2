...
const checkGuess = async (rgbGuess: number[], hexGuess: string): Promise<GuessResponse> => {
  ...
  originalRGB.forEach((number, i) => {
    ...
    guessNumberSplit.forEach((n, j) => {
      ...
      console.log(trackingDigitCount[n], "<--jhsdgfjhg"); */

      if (trackingDigitCount[n] > 0) {
        if (resultsArray[i][j] === "invalid") {
          trackingDigitCount[n]--;
          resultsArray[i][j] = "valid";
        }
      }
    });
  });
